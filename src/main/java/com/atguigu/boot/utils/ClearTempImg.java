...
public class ClearTempImg {
    ...
    public void run() {
        if (!isRunning) {
            log.debug("删除/Tomcat/Temp/目录下过期图片执行开始...");
            isRunning = true;
            log.debug("开始执行删除tomcat下temp文件夹png文件任务");
            log.debug("要删除图片文件所在路径为" + TEMP_PATH);
            File fileTemp = new File(TEMP_PATH); // 判断文件是否存在 
            boolean falg = false;
            falg = fileTemp.exists();
            if (falg) {
                log.debug("temp文件存在");
                if (true == fileTemp.isDirectory()) {
                    log.debug("temp文件是个目录");
                    ...
                    for (int i = 0; i < png.length; i++) {
                        if (true == png[i].endsWith("png")) {
                            ...
                            if (true == file.isFile()) {
                                ...
                                flag = file.delete();
                                if (flag) {
                                    log.debug("成功删除无效图片文件:" + file.getName());
                                }
                            }
                        }
                    }
                ...
                log.debug("未找到tomcat/temp文件夹，执行失败，请手动删除temp下所有png文件");
            }
            isRunning = false;
            log.debug("删除/Tomcat/Temp/目录下过期图片执行结束...");
        }
    }
}